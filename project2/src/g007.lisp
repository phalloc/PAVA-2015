...
(defmethod member? ((tensor1 vec) (tensor2 matrix))
	(let ((vec (make-array (length (vec-value tensor1)) :initial-element (s 1))))
		(loop for index from 0 below (length (vec-value tensor1))
			do (loop for lines from 0 below (first (matrix-dimensions tensor2))
					do (progn (loop for columns from 0 below (second (matrix-dimensions tensor2))
									do (if (eql (aref (vec-value tensor1) index)
												(aref (vec-value (aref (matrix-value tensor2) lines)) columns))
											(progn (setf (aref vec index) (s 1))
													(return))))
								(if (eql (aref vec index) 1)
									(return)))))
		(make-instance 'vec :value vec)))
		
(defmethod member? ((tensor1 matrix) (tensor2 vec))
	(let ((vec (make-array (second (matrix-dimensions tensor1)) :initial-element (s 0)))
		  (result (make-array (length (matrix-value tensor1)))))
		 (loop for lines from 0 below (length (matrix-value tensor1))
			do (progn (loop for columns from 0 below (second (matrix-dimensions tensor1))
							do (loop for index from 0 below (length (vec-value tensor2))
								do (if (eql (scalar-value (aref (vec-value (aref (matrix-value tensor1) lines)) columns))
													(scalar-value (aref (vec-value tensor2) index)))
											  (progn (setf (aref vec columns) (s 1))
											  (return)))))
						(setf (aref result lines) (make-instance 'vec :value vec))
						(setf vec (make-array (second (matrix-dimensions tensor1)) :initial-element (s 0)))))
		 (make-instance 'matrix :value result :dimensions (make-list-from-vec (vec-value (shape tensor1))))))

(defmethod member? ((tensor1 matrix) (tensor2 matrix))
	(let ((vec (make-array (second (matrix-dimensions tensor1)) :initial-element (s 0)))
		  (result (make-array (length (matrix-value tensor1)))))
		 (loop for lines from 0 below (length (matrix-value tensor1))
			do (progn (loop for columns from 0 below (second (matrix-dimensions tensor1))
							do (loop for lines2 from 0 below (length (matrix-value tensor2))
								do (loop for columns2 from 0 below (second (matrix-dimensions tensor2))
										do (if (eql (scalar-value (aref (vec-value (aref (matrix-value tensor1) lines)) columns))
													(scalar-value (aref (vec-value (aref (matrix-value tensor2) lines2)) columns2)))
											  (progn (setf (aref vec columns) (s 1))
											  (return))))))
						(setf (aref result lines) (make-instance 'vec :value vec))
						(setf vec (make-array (second (matrix-dimensions tensor1)) :initial-element (s 0)))))
